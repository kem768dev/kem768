...
6) Run only LicenseTests
   dotnet test .\src\LicenseTests\LicenseTests.csproj

7) Clean build outputs (bin/obj)
   Get-ChildItem -Recurse -Force -Directory -Filter bin, obj | ForEach-Object { Remove-Item -Recurse -Force $_.FullName }
...
- Warnings remain about Windows-only APIs in LicenseCore/LicenseSystem.cs (not related to PQ flow).
 - Unit tests include ML-KEM-768 round-trip, randomness, tamper detection, and a speed/size sanity test.
